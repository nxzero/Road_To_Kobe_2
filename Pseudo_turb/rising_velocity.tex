...
As demonstrated in the previous section, the droplet maintains an approximately spherical shape within the range of parameters studied here. 
...
From an experimental standpoint, quantifying the force exerted on ascending fluid particles in moderately dense conditions and establishing its correlation with the kinematics   properties of both the fluid and the particles poses considerable challenges. 
...
This is to be expected since this estimation is derived from the method of reflection and the aforementioned observation regarding the relative velocities of bubbles and solid particles. 
In contrast for a random free array the function $f$ can be expressed as  $f(\phi) = (1-b(\lambda)\phi)$ where $b$ is a function approaching the value $6.5$ for large $\lambda$ and $4.5$ for small $\lambda$ \citep{wacholder1973,haber1981}. 
...
Interestingly, \citet{loisy2017} observed a decrease of the suspension velocity in $\phi^{1/3}$ in similar ranges of Reynolds number. 


The results presented above are limited to very dilute configurations ($\phi \lesssim 5 \%$). 
However, in practical applications and especially in liquid-liquid extraction the volume fraction of the dispersed phase can be as high as $20\%$. 
To address moderately dense regimes, the current engineering approach involves resorting to empirical correlations such as the one developed by Richardson and Zaki  \citep{richardson1954},
\begin{equation}
f(\phi) = (1-\phi)^n
\label{eq:Richardson} 
\end{equation}
...
To represent this effect, we suggest the following correlation, 
\begin{equation}
...
Indeed, special treatments are essential in this regime to ensure that the numerical results are independent of the number of inclusions \citep{mo1994}.
...
Unfortunately, in contrast to the Stokes regime, there is no theoretical formula for the drag force on an isolated droplet for arbitrary Reynolds numbers. 
One may use potential flow theory (except in an infinitely thin boundary layer developing on the bubble surface) to predict the drag force on an isolated droplet in the limit  $Re\gg 1$ \citep{harper1968}, but numerical computations have shown that the theory is only accurate for $Re\geq 200$ \citep{dandy1989}.
Hence, in practice one has to rely on empirical relationships to predict the drag force on the drop. 
\citet{rivkind1976flow} proposed to write a drag force as a combination of the drag force on a solid spherical particle ($\lambda \rightarrow \infty$), and the drag force on a spherical bubble ($\lambda \rightarrow 0$)
\begin{equation}
...
In the Newtonian regime \ref{eq:stokes} becomes to \citep{jackson2000}, 
\begin{equation}
    n_p f_p = A (u_p - u_f)^2. 
...
The $\lambda$-weighted average of both formulas, given by \ref{eq:CD0}, has been validated in prior studies \citep{rivkind1976flow}. 
...
As mentioned earlier, the only unjustified assumption in this development is the formulation provided by \ref{eq:final_n} for high $Re$ and a viscosity ratio close to one of non-viscous droplets or bubbles ($\lambda < 10$). 
...
However, for $\phi = 0.2$ and $Re \approx 50$ slight discrepancies can be observed between the DNS results and \ref{eq:C_d_finalRe}. 
...
\begin{figure}[h!]
    ...
    \caption{
        \textit{Reynolds} number based on the superficial velocity divided by the \textit{Reynolds} number of an isolated particle in stokes flow $Re_\text{stokes}= \frac{8}{Re} \frac{3\lambda +2}{\lambda+1}$, in terms of the volume fraction $\phi$, for various viscosity ratios, (left) $\lambda  = 10$ (middle) $\lambda =1$ and (right) $\lambda = 0.1$. 
        The symbols represent the different \textit{Galileo} numbers. 
        (solid lines) Theoretical prediction form \ref{eq:CD0}. 
        (dot dashed lines) Theoretical prediction form \ref{eq:CD0} at $Re = 0$ corresponding to the relation \ref{eq:Richardson} with $n$ given by \ref{eq:n}. 
    }
    \label{fig:Reasim}
\end{figure}
...
Indeed, our prediction at $Ga =5$ already exhibits large differences in slops with the Stokes prediction given by: $(1-\phi)^{n(Re=0)}$. 
...
This means that the extension of \ref{eq:n_solid} for an arbitrary viscosity ratios $\lambda$, i.e. the formula \ref{eq:final_n}, seems indeed validated at these \textit{Reynolds} numbers that ranges between $45$ and $128$ for these cases.  
However, at this stage, it is difficult to confirm the $2.4$ constant of \ref{eq:final_n} for arbitrary $\lambda$, as it would require reaching significantly higher \textit{Reynolds} numbers ($Re > 500$) to enter this regime (see \ref{eq:n_jackson}). 
