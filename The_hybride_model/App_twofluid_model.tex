\subsection{Averaged the two-fluid model}
\label{ap:two-fluid_model}
Before introducing the \textit{hybrid model} which will be the subject of the next section, we first expose the classic averaged two-fluid model. 
...
\begin{align}
    ...
    + \div (
        \rho E \textbf{u}
        + \bm{q}^\text{eq}
        + \textbf{u} \cdot \bm{\sigma}^\text{eq}
        % - \textbf{u}_k^0 \cdot \bm{\sigma}_k^0 
        % + \textbf{q}_k^0
        )
    &= 
     \rho \textbf{u}  \cdot \textbf{g} 
\end{align} 
respectively. 
Where we have introduced the effective stress and heat fluxes namely, 
\begin{align*}
    &\bm{\sigma}^\text{eq}
    = 
     \rho\avg{ \textbf{u}'\textbf{u}'}
      -  \bm{\sigma},%- n_p \textbf{M}_p
    &\textbf{q}^\text{eq}
    =\textbf{q}^\text{e} +\textbf{q}^\text{k},  \\
    &\textbf{q}^\text{e}
    = \rho \avg{\textbf{u}' e'} 
    + \phi\textbf{q},
    &\textbf{q}^\text{k}
    = \rho \avg{ \textbf{u}' k} 
    - \avg{ \textbf{u}' \cdot \bm{\sigma}^0}.
\end{align*}
Where we have defined the total energy as, 
\begin{equation}
    E = \avg{\sum_k \rho_k [e_k^0 + \frac{1}{2}(u_k)^2] 
    + \delta_I \gamma}
    = e + k + \frac{1}{2}(u)^2
\end{equation}
with, $e = \sum_k \phi_k e_k + \phi_I \gamma$ and $k = \avg{\rho \textbf{u}'\cdot \textbf{u}'}/2$. 
Notice particularly that the bulk stress is defined as, 
\begin{equation*}
    \bm\sigma^{eff}
    = 
    \phi_f\bm\sigma_f
    ...
    + \phi_I\bm\sigma_I
\end{equation*}
These equations are particularly interesting on an experimental point of view. 
Indeed, as in experiment we cannot distinguish between the details of both phase. 
For example in a Rheometer we are able to measure only for the \textit{Bulk stress} $\bm\sigma$ as it measure the stress response from the mixture phases. 
