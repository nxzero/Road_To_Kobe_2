...
Additionally, it is interesting to note that while $\textbf{u}_m$ is not divergence-free \eqref{eq:dt_avg_rho}, the volume-averaged velocity fields $\textbf{u} = \phi_f \textbf{u}_f + \phi_d \textbf{u}_d $ is. 
Indeed, adding $\pddt \chi_k + \div \textbf{u}_k^0\chi_k =0$ for $k=f,d$ and ensemble averaging the resulting expression leads to 
\begin{equation}
    \div\textbf{u}=0.
\end{equation}
Thus, one might either solve the \textit{single-fluid} formulation for $\textbf{u}_m$ or reformulate the problem in terms of $\textbf{u}$, in which case the velocity fields become divergence-free. 
