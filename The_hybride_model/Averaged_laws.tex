...
However, in order to obtain a coarse-grained level equation for the dispersed phase, one must apply the average operator \ref{eq:avg} on \ref{eq:avg_dt_dq_alpha_tot} and \ref{eq:avg_dt_dQ_alpha_tot} which is made possible since the particle fields $\delta_\alpha \ldots$ are now defined over the whole space $\Omega$ thanks to the Dirac delta function.  
These equations yiedls,
\begin{align}
