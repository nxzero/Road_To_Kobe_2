...
\JL{In the sense of generalized functions, the partial time derivative of $\delta_\alpha$ can be written as
\begin{equation}
\frac{\partial \delta_\alpha(\textbf{x},t,\FF)}{\partial t} =  \frac{\partial \textbf{x}_\alpha}{\partial t} \cdot \frac{\partial \delta_\alpha}{\partial \textbf{x}_\alpha} .
\end{equation}}
...
Consider a domain with $N$ particles.\JL{pourquoi ne pas considerer la notation de Daniel $\delta _p$} We define the \textit{particle field} of a quantity $q_\alpha$​ as the sum of $\delta_\alpha q_\alpha$ over all particles in the domain, represented by $\displaystyle\sum_{\alpha=0}^N \delta_\alpha q_\alpha$​. 
...
\begin{equation}
     ...
     \label{eq:p_avg}
\end{equation}
\JL{where, $n_p(\textbf{x},t) = \avg{\sum_\alpha \delta_\alpha}$ is the probability density of finding a particle center of mass in the infinitesimal volume $d\textbf{x}$ around \textbf{x}, and $q_p(\textbf{x},t)$ is the average of $q_\alpha$ conditionally on the presence of a particle at \textbf{x} and time $t$. }
...
\JL{attention aux notations: "tot"...}
...
\end{align}
with, 
\JL{est on obliger de noter cela $\Phi_p^\text{eq-1}$ ? et que veut dire le $-1$ d'ailleurs}
...
The derivation of the higher moment particle-averaged equations is provided in \ref{ap:Moments_equations} \JL{separer l'annexe C en deux}. 
