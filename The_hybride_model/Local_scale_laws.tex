...
A second equation is obtained by taking the gradient of the distribution $\delta_I$ (\ref{ap:delta_I}) 
...
\begin{equation}
    \grad\delta_I 
    =   \grad (\textbf{n} \delta_I) \cdot \textbf{n}
...
Similarly, let $f_I^0$ represent an arbitrary surface property defined on $\Gamma$. The subscript $_{||}$ is used to denote the projection of a field onto the plane tangential to the surface $\Gamma$. Specifically, for an arbitrary quantity $\textbf{g}$ defined on $\Gamma$, its tangential projection is given by $\textbf{g}_{||} = \textbf{I}_{||}\cdot \textbf{g}$ where $\textbf{I} _{||} = (\textbf{I}-\textbf{nn})$ is a tangential projection operator and $\textbf{I}$ is the identity tensor. This definition also extends to the gradient operator, where $\gradI= \textbf{I} _{||}\cdot \grad$ is referred to as the surface gradient operator. In addition we may define the surface divergence operator as $\gradI \cdot ()= (\textbf{I} _{||}\cdot \grad)\cdot()$.
