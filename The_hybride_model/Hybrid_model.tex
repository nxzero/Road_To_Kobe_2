...
\begin{align}
    ...
    -  \pSavg{\textbf{q}_1^0 \cdot \textbf{n}_2}\nonumber\\
    + \textbf{u}_p \cdot\pSavg{{\bm{\sigma}_1^0 \cdot \textbf{n}_2}}
    + \pSavg{{\textbf{u}_\alpha' \cdot\bm{\sigma}_1^0 \cdot \textbf{n}_2}}
...
Now that all the equations are properly displayed we can stipulate that the only explicit role of the surface tension coefficient and the particle internal stress is within in the symmetric moment of momentum equations and inside the internal kinetic energy equation. 
These, equation will be discussed in more detail in \ref{sec:Exemples}. 


\subsection{Comments on the general closure problem}

Now that all the equations are exposed we expose the closure problem. 
Firstly, we find all the closure related to the microscale fluctuation, 
\begin{equation*}
    \avg{\chi_1 \rho_1 \textbf{u}_1'\textbf{u}_1'}, \;\;
    \avg{\chi_1 \rho_1 \textbf{u}_1'e_1'}, \;\;
    \avg{\chi_1 \rho_1 \textbf{u}_1'k_1'}, \;\;
    \avg{\chi_1 \rho_1 \textbf{u}_1'\cdot \bm{\sigma}_1^0}, \;\;
    % \avg{\chi_1 \rho_1 \textbf{u}_1'\textbf{u}_1'}, \;\;
\end{equation*}
\begin{equation*}
    \pavg{\rho_2 \textbf{u}_\alpha'\textbf{u}_\alpha'}, \;\;
    \pavg{\rho_2 \textbf{u}_\alpha' e_\alpha'} , \;\;
    \pavg{\rho_2 \textbf{u}_\alpha' W_\alpha'} , \;\;
    \pavg{\gamma \textbf{u}_\alpha' s_\alpha'} , \;\;
    \pavg{\rho_2 \textbf{u}_\alpha' k_\alpha} , \;\;
    \pavg{ \textbf{u}_\alpha' \mathcal{M}_\alpha'} , \;\;
    \pavg{ \textbf{u}_\alpha' \mathcal{P}_\alpha'} .
\end{equation*}
Where the first row represent the closure related to the fluid phase and the second to the particle phase. 

The second type of closure is the terms that makes the exchange between the carrier fluid and the particle. 
Among the exchange terms we can firstly list the zero order exchange term, namely,
\begin{equation*}
    \pSavg{\bm{\sigma}_1^0\cdot \textbf{n}_2} , \;\;
    \pavg{\textbf{u}_\alpha' \cdot \intS{\bm{\sigma}_1^0\cdot \textbf{n}_2}} , \;\;
    \pavg{\intS{\textbf{w}_2^0 \cdot \bm{\sigma}_1^0\cdot \textbf{n}_2}} , \;\;
    \pSavg{\textbf{q}_1^0\cdot \textbf{n}_2} , \;\;
\end{equation*}
which correspond to, the drag force, the drag force velocity correlation, the surface velocity surface force correlation and the heat flux. 
Each of these terms have a first order moment counterpart which appear in the fluid phase equations as, 
\begin{equation*}
    \pSavg{\textbf{r}\bm{\sigma}_1^0\cdot \textbf{n}_2} ,\;\;
    \pavg{\textbf{u}_\alpha' \cdot \intS{\textbf{r}\bm{\sigma}_1^0\cdot \textbf{n}_2}} ,\;\;
    \pavg{\intS{\textbf{r}\textbf{w}_2^0 \cdot \bm{\sigma}_1^0\cdot \textbf{n}_2}} ,\;\;
    \pSavg{\textbf{r}\textbf{q}_1^0\cdot \textbf{n}_2} ,\;\;
\end{equation*}
which correspond to the first hydrodynamic moments, the correlation between this moment and the center of mass velocity, the first moment of the surface work and the first moment of the heat fluxes. 
All other kind of closure is volume averaged terms found in the energy and first moment of momentum equations, namely,
\begin{equation*}
    \pOavg{{\bm{\sigma}_2^0 : \grad\textbf{u}_2^0}}, \;
    \pOavg{\rho_2 \textbf{w}_2^0\textbf{w}_2^0}, \;
    \pOavg{\bm{\sigma}_2^0}, \;
    \pOavg{\gamma \textbf{I}_{||}}, \;
    \avg{\chi_1 \bm{\sigma}_1^0 : \grad\textbf{u}_1^0},\;    
    s_pn_p\gamma, \;
    \phi_1 \bm{\sigma}_1, \;
    \phi_1 \textbf{q}_1. 
\end{equation*}
A fully closed model must seek for expressions which links every of the closure presented above, to any of the unknown that we solve the averaged equations for.
In other words, we must formulate each closure terms in terms of : $\phi_1$, $n_p$, $\textbf{u}_1$, $\textbf{u}_p$, $k_1$, $k_p$, $W_p$, $e_1$, $e_p$, $\mathcal{M}_p$, $\mathcal{P}_p$. 
To conclude, the set of equations formed by \ref{eq:dt_hybrid_rho}, \ref{eq:dt_hybrid_rhou_1}, \ref{eq:dt_hybrid_k1}, \ref{eq:dt_hybrid_e1} for the fluid phase, and \ref{eq:dt_hybrid_mp}, \ref{eq:dt_hybrid_up}, \ref{eq:dt_hybrid_kp}, \ref{eq:dt_hybrid_Wp} and \ref{eq:dt_hybrid_ep} for the particle phase, together a consistent expression of the closure listed above provide us with a first order description of the dispersed phase. 
Note that the needs for this first order moments is really problem depends, this idea is developed in \ref{sec:Exemples}. 

\subsection{The fluid phase equivalent stress}

% In this section we focus on the formulation of the averaged fluid phase equivalent stress tensor $\bm{\sigma}_1^\text{eq}$. 
...
Additionally, we state that the fluid strain is equal to the bulk strain $\textbf{e} = \grad \textbf{u}+ (\grad \textbf{u})^T$, minus the particle averaged strain, i.e. $\phi_1 \mu_1 \textbf{e}_1 = \mu_1\textbf{e} - \phi_2 \textbf{e}_2$. 
...
Upon developing $\phi_2 \textbf{e}_2$ multipolar series and making use of teh divergence theorem on the first term, the equivalent stress of the fluid phase can be reformulated as, 
\begin{equation*}
    \bm{\sigma}^\text{eq}_1 = 
    + \phi_1 p_1 \textbf{I} 
    - \mu_1 \textbf{e} 
    ...
    + \mu_1 \pSavg{\textbf{u}_1^0 \textbf{n}_1 + \textbf{n}_1 \textbf{u}_2^0}
    - \div \pMOavg{\mu_1 \textbf{r} \textbf{e}_2 }
    \label{eq:sigma_eq_0}
\end{equation*} 
...
The particle contribution to the suspension stress is then represented by the multipolar expansion of the traction forces $\pMSavg{\textbf{r}\bm{\sigma}_1^0\cdot \textbf{n}_2}$ minus the internal strain rate multipolar expansion $\mu_1 \pSavg{ \textbf{u}_1^0 \textbf{n}_1 + \textbf{n}_1 \textbf{u}_2^0} - \div \pMOavg{\mu_1 \textbf{e}_2 \textbf{r} }$. 

% By ex $\textbf{e}_1$,  we can express the shear stress as, the continuous phase averaged strain. 
% \begin{equation*}
%     \phi_1 \bm{\tau}_1
%     = \mu_1 \textbf{e} - \mu_1 \phi_2 \textbf{e}_2
%     = \mu_1 \textbf{e} 
%     % -\mu_1 \avg{\chi_2 (\grad\textbf{u}_2^0 +  \grad\textbf{u}_2^0)}
%     - \mu_1\pSavg{\textbf{n}_2 \textbf{u}_2^0 +  \textbf{u}_2^0\textbf{n}_2 }
%     + \div \pOavg{\mu_1 \textbf{r}\textbf{e}^0_2}
%     + \ldots
%     \label{eq:tau_1_final}
% \end{equation*}
% where this relation simply state that the fluid strain is equal to the bulk strain $\textbf{e} = \grad \textbf{u}+ (\grad \textbf{u})^T$, minus the particle averaged strain $\textbf{e}_2$ Taylor expansion. 
% Under this form we clearly see that all terms except the first one in \ref{eq:tau_1_final} vanish for solid particles motion, recovering the expression $\phi_1 \textbf{e}_1 = \textbf{e}$. 
...
\begin{multline*}
    ...
    + \div  \underbrace{\left(\pMSavg{\textbf{rr} \bm{\sigma}_1^0\cdot \textbf{n}_2}
    -\pMOavg{\mu_1 \textbf{r} \textbf{e}_2 }\right)}_\text{inhomogeneous particles stresses}
