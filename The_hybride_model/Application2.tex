...
\tb{mettre vraiment les equations de base c.a.d :}
\begin{align}
...
Note that in \ref{eq:two_fluid_momentum} or \ref{eq:two_fluid_momentum} we used a different definition for $\bm\sigma_f'$, which is typically defined as $\bm\sigma_f'= \bm\sigma_f^0 - \bm\sigma_f = \bm\sigma_f^0 - \bm\Sigma_f + \mu_f/\phi_f \avg{\delta_\Gamma (\textbf{u}_f'\textbf{n} + \textbf{u}_f'\textbf{n}_d)}$ \citep{zhang1997momentum}.
\tb{because $\avg{\delta_\Gamma}\sim \phi$ the last term contribute to $\phi^2$ in the final expression of the force. } 
...
\subsubsection{Twoard a $\mathcal{O}(Ca)$ model}



Drag force in term of the deformation. 
\subsubsection{On the effect of inertia}
