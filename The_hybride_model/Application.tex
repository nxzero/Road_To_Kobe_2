...
Note that the last term of \ref{eq:stress_closure_zhang} is proportional to the interface area concentration, hence it is at most of $\mathcal{O}(\phi_d)$.
Thus, when integrating $\bm\sigma_f'$ on the interface of the droplets, the contribution from that term to the mean momentum exchange term is at most of $\mathcal{O}(\phi_d^2)$. 
...
%since we are working at $\mathcal{O}(\phi_d)$.
...
Note that to obtain averaged equations accurate at $\mathcal{O}(\phi)$, it is sufficient to consider a closure problem accurate at $\mathcal{O}(1)$  in $\phi$ \citep{hinch1977averaged,zhang1994averaged}, hence neglecting droplets interactions.
...
Each averaged properties may possess different variation length scale, for instance think about $\grad \textbf{u}$ which typically varies at the length scale of the process, while $\grad\gamma$ may vary according to smaller length scale. 
For example if the gradient of surface tension is generated by transport of surfactants it typically varies over the length scale of $\mathcal{O}( a)$. 
If $\gamma$ varies because of non-constant temperature field then the length scale of variation will be the one of the temperature fields which is related to process or macro scale. 
To avoid complication we consider in our problem that $\grad\gamma$, and $\grad \textbf{u}$ typically varies according to $\sim L$. 
Then, we consider that all contribution proportional to $O(a^2/L^2)$ may be negligible in the averaged equations \citep{jackson1997locally,zhang1997momentum}. 


A summary of the dimensionless parameters is given in \ref{tab:dimensionless_para}. 
% \tb{ 
...
Note that we have neglected the terms proportional to $\grad^2 \textbf{E}$ in the first moment expression, and to $\grad\grad \textbf{u}$ or $\grad\grad\grad \gamma$ in the second moment expression, as they are of order $\mathcal{O}(a^2/L^2)$ or smaller.
...
\end{align}
We have used the approximation of $\bm\Sigma_f(\textbf{x}+\textbf{r}) = \bm\Sigma_f(\textbf{x}) + \textbf{r}\cdot\grad \bm\Sigma_f|_{\textbf{r}=0} + \ldots$ and neglected the $\mathcal{O}(a^2/L^2)$ terms. 
...
As a result, only the first two terms in \ref{eq:Reynolds_stress_functional_form} are significant in the momentum equation\footnote{
    Note that since $\textbf{u}_f' \propto \frac{a \grad \gamma}{\mu_f}$ it follows that $\textbf{u}_f'\textbf{u}_f' \propto \mathcal{O}(a^2/L^2)$ and is therefore negligible in the current modeling hypothesis. 
}.
...
Remark that at $\mathcal{O}(1)$ in $Ca$ the linear momentum equations are not coupled with the dispersed phase moments ($\textbf{S}_p,\bm\mu_p$, and $\textbf{M}_p$).  
...
Upon the precise knowledge of $C^1_{up}, C^2_{up}, C^1_{uu}$ and $C^2_{uu}$ one may state that this system is closed accurate at $\mathcal{O}(\phi)$. 
...
Now, we demonstrate how higher-order moments equations \eqref{eq:dt_hybrid_Sp,eq:dt_hybrid_Mp,eq:dt_hybrid_mup} can characterize droplet shapes and relate them to the averaged equations governing the continuous phase.
As we  see now, with closure terms accurate at $\mathcal{O}(1)$ in $Ca$ one can use \ref{eq:dt_hybrid_Sp} to compute the droplet deformation at $\mathcal{O}(Ca)$. 
...
Accurate at $\mathcal{O}(Ca)$ the mass, second moment of mass, and the traceless part of the first moment of surface forces, can be computed and read as,
\begin{align}
    n_p m_p &= \rho_d \phi   
    \label{eq:volume}
    \\ 
...
Then, to derive \ref{eq:closure_surface_tension2,eq:closure_surface_tension3,closure_surface_tension4} we have used the approximation $\textbf{n} = \textbf{e} - Ca (\bm\delta - \textbf{e}\textbf{e})\cdot \grad f_\alpha + \mathcal{O}(Ca^2)$ and injected the expression of $f_\alpha$ given by \ref{eq:f_definition}.  
As witnessed by \ref{eq:volume}, the definition given by \ref{eq:parametrization} ensures that the droplets preserve a constant volume regardless of the values of the $\textbf{H}_p^{(n)}$. 
...
\textbf{S}_p &= 0 \label{eq:S_eq_zerp}\\
    ...
    \textbf{H}_p^{(2)}
    - \frac{1}{5}(\textbf{H}_p^{(2)}\cdot\grad\grad \gamma)^\text{sym} 
    +
    \frac{30}{56}\frac{a}{\gamma}\grad\gamma\cdot \textbf{H}^{(3)}_p
    + \frac{2}{5}\textbf{H}_p^{(4)}:\grad\grad \gamma
    \nonumber
    &=
...
\end{align}
respectively. 
Note that at $\mathcal{O}(1)$ in $Ca$ the derivative of $\textbf{M}_p$ are zero (we thus deduce \ref{eq:S_eq_zerp}), hence the droplet shape behave as if it was computed in a quasi-steady-state regime.
Therefore, at $\mathcal{O}(1)$ in $Ca$ we observe a ``one-way'' coupling between the droplet deformation, given by \ref{eq:last}, and the mass and momentum equations which drives $\textbf{u}_f$. 
Discarding the surface tension gradient one recover the original results of \citet{taylor1932viscosity,rallison1984deformation,nadim1996concise} for the droplet deformation in pure linear flow\footnote{Note that the deformation of the droplets due to the mean cubic flow, ($\grad^2\textbf{E}$), could also be included here using Faxen relations on the first moments of hydrodynamic forces.
Nevertheless, this term is $\mathcal{O}(a^2/L^2)$ higher than the other and thus negligible in this contex.t }.  
...
% Note that the second term correspond to the ``Faxen''-like contribution to the deformation of the droplet, nevertheless as expected this term is $\mathcal{O}(a^2/L^2)$ higher than the other and thus negligible in this context.  
...
This term is of order $\mathcal{O}(a^2 \grad\grad \gamma/\mu_f U)$ representing the dimensionless second-order gradient of capillary forces. 
...
For instance to determine $\textbf{H}_p^{(3)}$ which is needed by \ref{eq:def_H2} one must use the second moment of momentum equation and inject the closures for the second moment of momentum, a first moment of the internal shear rate. 
As the calculation is quite lengthy the details are reported in appendix.
% Additionally, because it doesn't yield any particular interest we neglect the effects of gradient of $\gamma$ in this expression. 
The result yields, 
\begin{align}
    \label{eq:defH3}
    Ca \frac{8}{7} \gamma \textbf{H}_p^{(3)}
    + \mathcal{O}(\grad\gamma\cdot \textbf{H}_p^{(4)})
    &+ \mathcal{O}(\grad\gamma \textbf{H}_p^{(2)})
    =\\
...
\end{align}
Note that in \ref{eq:defH3} the tensor $\textbf{H}_p^{(4)}$ and  $\textbf{H}_p^{(2)}$ appear because of the non-zero $\grad\gamma$ however the exact expression is omitted for instance.
Thus, even considering only first order derivatives in $\gamma$ one is still constrained to consider an infinite series of harmonic equations.  
Because this term is $\propto a^2$ it may be neglected in our current modeling hypothesis. 

It is clear that $\textbf{u}_r$ and $\grad \gamma$ doesn't actually play a role in the deformation of the drop. 
To make this clear, consider a force free droplet, then according to the momentum balance one may write,
\begin{equation}
    0 = 
    \frac{\mu_f}{a^2}
    \frac{3(2+3\lambda)}{2(1+\lambda)}\textbf{u}_r
...
\end{equation}
which can be used in \ref{eq:defH3} to find, 
\begin{multline}
    \textbf{H}_p^{(3)}
    + \mathcal{O}(\grad\gamma\cdot \textbf{H}_p^{(4)})
    + \mathcal{O}(\grad\gamma \textbf{H}_p^{(2)})
...
Both of this contribution end up begin of $\mathcal{O}(a^2/L^2)$ or higher in this equation. 

Thus, at this order of accuracy one may neglect $\textbf{H}_p^{(3)}$ in \ref{eq:def_H2}  and invert the equation for $\textbf{H}_p^{(2)}$ and find an explicit formula for the deformation. 

To reach an accuracy of $\mathcal{O}(Ca)$ one must find closures laws for \ref{eq:drag_forces} to \ref{eq:secondUN} in terms of the deformation defined by \ref{eq:f_definition}. 
