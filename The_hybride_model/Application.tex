...
However, to point out the meaning of the first-moment of momentum equation we introduce an alternative approaches based on these Lagrangian balance laws \eqref{eq:dt_hybrid_Sp,eq:dt_hybrid_Mp,eq:dt_hybrid_mup}. 
...
Then, to derive \ref{eq:closure_surface_tension2,eq:closure_surface_tension3,eq:closure_surface_tension4} we have used the approximation $\textbf{n} = \textbf{e} - Ca (\bm\delta - \textbf{e}\textbf{e})\cdot \grad f_\alpha + \mathcal{O}(Ca^2)$ and injected the expression of $f_\alpha$ given by \ref{eq:f_definition}.  
...
For instance to determine $\textbf{H}_p^{(3)}$ one must use the second moment of momentum equation (see \ref{eq:second_mom}) and inject the closures for the second moment of hydrodynamic forces as well as for the first moment of internal shear rate. 
As the calculation is quite lengthy the details are reported in appendix (see \ref{ap:singularity_solution}), and, because it doesn't yield any particular interest for the discussion we neglect the effects of buoyancy force in this expression. 
The result yields, 
\begin{multline}
    \label{eq:defH3}
    Ca \frac{8}{7} \gamma \textbf{H}_p^{(3)}
    + \mathcal{O}(\grad\gamma\cdot \textbf{H}_p^{(4)})
    + \mathcal{O}(\grad\gamma \textbf{H}_p^{(2)}) + \ldots
    =\\
...
\end{multline}
Note that in \ref{eq:defH3} the tensor $\textbf{H}_p^{(4)}$ and  $\textbf{H}_p^{(2)}$ appear because of the non-zero $\grad\gamma$, however the exact expression as well as higher order gradients are omitted here.
% Thus, even considering only first order derivatives in $\gamma$ one is still constrained to consider an infinite series of equations for the $\textbf{H}_p^{(n)}$.  
% Because this term is $\propto a^2$ it may be neglected in our current modeling hypothesis. 

It is clear that $\textbf{u}_r$ doesn't actually play a role in the deformation of the drop\citep{nadim1991motion}, and neither do $\grad\gamma$ as we show now. 
To make this clear, consider the force balance on a force-free droplet, then according to the momentum balance one may write,
\begin{equation}
    0 = 
    \frac{\mu_f}{a^2}
    \frac{3(2+3\lambda)}{2(1+\lambda)}\textbf{u}_r
...
Thus, at this order of accuracy one may neglect $\textbf{H}_p^{(3)}$ in \ref{eq:def_H2}  and invert the equation for $\textbf{H}_p^{(2)}$ and find an explicit formula for the deformation in terms of the gradient of $\gamma$ and \textbf{E}. 
...
\tb{+perspective sur les grad force}
