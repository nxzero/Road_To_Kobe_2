...
By applying \eqref{eq:avg_dt_chi_f} with $f_k = \rho_f$ and $\rho_f\textbf{u}_f$ we obtain the mass and momentum equation for the continuous phase under the two fluid formulation, 
\begin{align}
...
Second, employing \ref{eq:two_fluid_momentum} with \ref{eq:disturbance_stress1} may appear more convenient as the governing equations are directly formulated in terms of the fluid velocity field $\textbf{u}_f$, thereby circumventing the need to explicitly include an expression for the bulk velocity ($\textbf{u}$). %the following equation
...
%The closure terms in the above set of equations are expressed in terms of $p_f'$ and $\textbf{u}_f^*$, therefore we are seeking the disturbance velocity and pressure fields generated by a spherical droplet immersed in an arbitrary flow. 
...
%Additionally, we neglect inertia in the closure problem, meaning that we neglect all the terms of $O(Re)$ in the closure problem, and all the terms of $O(Re\phi)$ in the averaged equations (the dimensionless parameters are summarized in \ref{tab:dimensionless_para}). 
...
%Note that a ``Faxen-like'' contribution of the Marangoni forces exist and involves the third derivative $\gamma$ but is negligible to $O(a^2/L^2)$.
...
%Hence, one can simply inject \ref{eq:drag_forces}-\ref{eq:mean_contributions}, into  \ref{eq:dt_hybrid_up} and \ref{eq:dt_uf2} to obtain a closed form of the hybrid model, namely,  
...
\end{align}
%Indeed,
%Accurate at $O(Ca)$ the mass, second moment of mass read as, 
...
%What has been unoticed before is that terms of the form $\sim \grad\grad\gamma$ seem to induce stresses in the suspension as well.
...
%As we see now, with the closure derived in the previous section one can use \ref{eq:dt_hybrid_Sp} to compute the first order correction to the spherical shape.
%In other word  with closure terms accurate to $O(Ca)$ the $Ca$ droplet deformation. %at $O(Ca)$. 
%The standard approach to determining droplet deformation involves applying the normal stress balance \citep{nadim1991motion,nadim1996concise}. 
...
%, which reads
%In the low Reynolds number limit
\begin{align}
\pSavg{\gamma (\frac{1}{3}\bm\delta - \textbf{nn})}
=&- \pOavg{2 \mu_d\textbf{e}_d^*} \nonumber
+\pSavg{\frac{1}{2}(\textbf{r}\bm\sigma_f^*+^\dagger\textbf{r}\bm\sigma_f^*-\frac{2}{3}(\bm\sigma_f^* \cdot \textbf{r})\bm\delta)\cdot \textbf{n}}\nonumber\\
...
We first Taylor expand the surface tension about the point $\textbf{x}$ as $\gamma |_{\textbf{x}_\alpha} = \gamma|_\textbf{x}+\textbf{r}\cdot \grad \gamma|_\textbf{x}+\frac{1}{2}\textbf{rr}:\grad\grad\gamma|_\textbf{x}+\ldots$ to get 
 %also need closures for the traceless part of the first moment of surface forces, can be computed
\begin{equation}
\pSavg{\gamma (\bm\delta/3 - \textbf{nn})} = \gamma \pSavg{(\bm\delta/3 - \textbf{nn})} +\grad \gamma \cdot  \pSavg{\textbf{r}(\bm\delta/3 - \textbf{nn})} + \ldots
%\pSavg{(\gamma|_\textbf{x}+\textbf{r}\cdot \grad \gamma|_\textbf{x}+\frac{1}{2}\textbf{rr}:\grad\grad\gamma|_\textbf{x}+\ldots) }. \\
...
%to derive \ref{eq:closure_surface_tension2,eq:closure_surface_tension3,eq:closure_surface_tension4} 
\begin{align}
...
%The first effect of surface tension gradient is that $\textbf{H}_p^{(3)}$, and all the other $\textbf{H}^{(n)}_p$,  appears on the left-hand side of \ref{eq:def_H2}.
%Hence coupling the equation of $\textbf{H}_p^{(2)}$ with the other $\textbf{H}_p^{(n)}$. 
...
%However one may perfom several simplification by noting first that all the terms in $\textbf{H}_p^{(n)}$ with $n\geq 4$ are of order $a^2/L^2$ or smaller.
%The same comment apply on the second term on the right-hand side which is of order $a^2/L^2$.
...
%Then the effect of surface tension gradient is $a/L$ smaller than the effect of strain but not neglgiible in the current framework.
%in the determination of the shape of the droplet, which is solely described by the tensor $\textbf{H}_p^{(2)}\sim \textbf{E}$. 
...
All terms contributing to $\textbf{H}_p^{(3)}$ are of order  $O(a/L)$, implying that in \ref{eq:H2_simplified} the term involving $\textbf{H}_p^{(3)}$ scales as $O(a^2/L^2)$ and is therefore negligible. %the term involving $\textbf{H}_p^{(3)}$ is of $O(a^2/L^2)$, hence negligible.
...
%If we consider the term which scales as $O(a^2/L^2)$ the problem becomes considerably more complicated for two reasons: (1) new sources terms related to gradients of $\gamma$ appear on the right-hand side of \ref{eq:def_H2} and \ref{eq:defH3}, and (2) the equations for each $\textbf{H}_p^{(n)}$ become interconnected.  
