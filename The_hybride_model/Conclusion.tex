...
\item \tb{
    Note that in the continuous phase conservation equation \eqref{eq:avg_hybrid_dt_chi_f} an infinite number of moment of the interphase exchange term are involved. 
    Hence, the last reason why one may need to obtain such higher moment is because it is required directly by \ref{eq:avg_hybrid_dt_chi_f}. 
    The most relevant example, is the first moment of momentum exchange term that is responsible for the Einstein's viscosity correction, when one  \citet{}.
    Additionally, note that the second moment of momentum exchange turns out to be non-negligible as well \citep{lhuillier2009rheology}. 
}
\end{enumerate}
