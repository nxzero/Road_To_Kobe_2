...
\begin{align}
    ...
    \label{eq:u_alpha}
\end{align}
Making use of the conservation of mass the firt term on the second line of \ref{eq:u_alpha} vanishes. This yields,%(\ref{eq:dt_rho}) and the definition of $\textbf{x}_\alpha(t,\FF)$ in the last term, gives
\begin{equation}
    ...
    -  \frac{\textbf{x}_\alpha}{m_\alpha}    \intS{ \rho_d(\textbf{u}_I   - \textbf{u}_d^0) \cdot \textbf{n}_d }
\end{equation}
Then by considering the mass conservation for the first term and noticing that $\grad \textbf{x} = \textbf{I}$ gives%for the second term gives, 
\begin{equation}
...
To differentiate time-varying surface integrals with respect to time, we make use of the surface Reynolds transport theorem, whic reads \citep{nadim1996concise} %which states that for an arbitrary function $f_I^0$ defined on $\Gamma(t)$ the following relation holds \citep{nadim1996concise}
\begin{equation}
    \ddt  \intS{f_I^0 }
    = \intS{ \left[
        \frac{D f_I^0}{Dt}+
...
\end{equation}
\JL{Upon expanding the acceleration term and noting that $\partial f_I^0 /\partial n=0$ we obtain}
\begin{equation}
    \ddt  \intS{f_I^0 }
    = \intS{ \left[
        \pddt f_I^0
...
= 0$.
This yields,%explains why $\bm\Phi_{I||}$ does not appear in \ref{eq:dt_q_I_alpha}. 
...
%Of course, this holds only under the assumption that the non-convective flux is parallel to the interface.%$\mathbf{\Phi}_{I||}^0$ is parallel to the surface.
...
\JL{pq ne pas noter les moments $q_\alpha^{(0)}, q_\alpha^{(1)}, etc$. Comme ca on peut ensuite noter le "total" avec un grand $Q$.}

Because $f_d^0$ and $f_I^0$ are not always constant over the volumes and surfaces of the particles, it is interesting to introduce the first moment of the quantities $f_d^0$ and $f_I^0$. 
They are defined as $\textbf{Q}_\alpha     = \intO{ \textbf{r} f_d^0 }$ and $\textbf{Q}_{I\alpha}    = \intS{ \textbf{r} f_I^0 }.$ %, where
%\begin{align}
%    &\textbf{Q}_\alpha 
%    = \intO{ \textbf{r} f_d^0 },
%    &\text{and}&
...
For similar reason than the linear conservation equations, we sum \ref{eq:dt_Q_alpha} and \ref{eq:dt_Q_I_alpha} to express the conservation equation of the total first moment $\textbf{Q}_\alpha^\text{tot} = \textbf{Q}_\alpha + \textbf{Q}_{I\alpha}$, this yields 
\begin{multline}
...
%These higher order moments describe the distributions of the local quantities $f_d^0$ and $f_I^0$ inside the domain $\Omega_\alpha$ and $\Gamma_\alpha$, respectively.
