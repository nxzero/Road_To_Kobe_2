...
The shape of a droplet is entirely described by an orientation vector $\textbf{p}$ and its two semi-axis, $a_1$ and $a_2$.  
...
Thus, the \textit{transient internal velocity field} of a droplet can be modeled as $\textbf{w}_d^0 = \bm\Gamma_\alpha \cdot \textbf{r}$, where we have introduced, $\bm\Gamma_\alpha$, the mean velocity gradient inside the droplet, which symmetric part: $\textbf{E}_\alpha$, represents the rate of strain, and skew-symmetric part: $\bm\Omega_\alpha$, represents the angular velocity. 
...
Adopting these definitions, the droplet's internal velocity is decomposed as, 
\begin{equation}
...
With this definition $\textbf{v}_d^0$ represents the droplet's internal motion that does not contribute to the linear homogeneous deformation and angular rotation of the drop. 
...
Also, note that to preserve the spheroidal shape of the droplet we must assume that the principal direction of the rate of strain of the droplet is the same as the droplet's shape principal axis. 
...
As demonstrated in \ref{chap:closure-disperse} the internal motions of an isolated spherical drop, such as the one presented in  \ref{fig:flowlines1}, are entirely determined  from $\textbf{u}_f$,$\grad\textbf{u}_f$ and $\textbf{u}_\alpha$ and the shape of the drop. 
...
From now on we consider that the internal velocity $\textbf{v}^0_d(t,\textbf{r})$ is not part of the droplet' unknown, but rather as a closure term that should be expressed in terms of $\textbf{u}_\alpha$ $\textbf{M}_\alpha$, $\textbf{u}_f$ and $\grad\textbf{u}_f$. 
...
Now that the droplet's shape and internal kinematics   are properly defined we may simplify the moments of mass and momentum equations. 
...
\end{align}
where we have introduced the \textit{inertia moment} of the droplet $\alpha$ as $\textbf{I}_\alpha = (\bm\delta : \textbf{M}_\alpha)\bm\delta - \textbf{M}_\alpha$. 
...
Therefore, it corresponds to the description of the deviation from the spherical droplet's shape $\bm\chi_{\alpha,ij}$. 
...
Indeed, the angular velocity of \textbf{p} corresponds, by definition, to the droplet's angular velocity. 
...
In contrast, the droplet's rotation plays a significant role in the droplet's dynamical balance equations.
...
Thus, this coupling is caused by the effect of the droplet's internal pressure term. 
...
When projected onto the first eigenvector of the droplet's shape tensor \ref{eq:dev} simplifies to the classical model proposed by \citet{lamb1924hydrodynamics}, a model still employed in recent studies \citep{riviere2021sub}. 
...
% In short \ref{eq:dt_chi_I} demonstrates that when assuming a spheroidal shape for the droplets, the equation governing $\bm\chi_\alpha$ is a nonlinear tensor equation rather than the typical second-order linear PDE. 
