...
\begin{equation}
    P_\text{nst}^\text{th}(r) = n_p e^{- 8\phi\left[(r/d)^3-1\right]}.
...
Indeed, in the latter case $P_\text{nst}^\text{th} = 0$ for $r<d$ while in our case particles might deform at contact, meaning that $P_\text{nst}^n$ is finite for certain $r<d$. 
