...
%In the high inertial cases (\ref{fig:Pnst_high_Ga}), we can note that $P_\text{nst}$ is larger on the sides of the \textit{test particle} for the iso-viscous emulsions ($\lambda = 1$).
...
%In comparison the high viscosity drops show an isotropic discitrbution of nearest droplet around the \textit{test droplet}. This could witness of the presence of isotropic clustering of droplets.
...
%In all rigor, DNS in a larger domain with more droplets would be required to evaluate the microstructure dependence on the domain size. 
...
\end{figure}
%In both cases 
In \ref{fig:torquato} we show that $P_\text{nst}^\text{th}(r)$ predict a more concentrated nearest neighbor concentration at the contact of the \textit{test droplet} ($r/d=1$) compared to the dilute random distribution $P_\text{nst}^{\phi\ll 1}(r)$. 
...
\begin{figure}[h!]
    ...
    \caption{
    Ratio of the radial probability density functions, $P_\text{r-nst}/P_\text{nst}^\text{th}$ with $P_\text{nst}^\text{th}$ given by \ref{eq:Pnst_dilute}, as functions of the dimensionless distance $r/d$, for $Ga = 100$, $Ga =10$, $\lambda = 1$ and $\lambda = 10$.
...
\begin{figure}[h!]
    ...
    \caption{
        (left) Dimensionless trace of $\textbf{R}$ as a function of the volume fraction.%Trace of the second moment of the probability density function $P_\text{nst}(\textbf{r})$ divided by the square diameter of the droplets $d^2$. 
    ...
    Black symbols represent the results of \citet{zhang2023evolution} for sedimenting spherical droplets with $\phi = 0.016,0.056,0.134,0.262$ and $\zeta = 2.56$. %$\phi = 0.0168,0.0565,0.1341,0.2622$ 
...
%Although, previous studies mainly focused on bubbles or solid droplets, it is reasonable to compare the $\lambda = 1$ and $\lambda = 10$ simulations, to the former and the latter cases, respectively. 
...
For solid droplets at $Ga = 144$, it is observed in \citet{shajahan2023inertial} that vertical rafts of droplets are formed in the dilute regime ($\phi \approx 0.02$). This phenomenon was attributed to a well-defined wake around individual droplets, which effectively traps neighboring droplets within the wake without causing repulsion toward the sides. %This effect was explained by the presence of a more developed wake for dilute solid droplets which trap neighboring droplets within the wake without repulsing it on the sides. 
In our case, we note a greater concentration of droplets in the vertical directions for $\lambda = 10$ compared to $\lambda = 1$ at $Ga = 100$, as evidenced by the smaller value of $A_{xx}$ in the former case. Although not immediately apparent, this phenomenon could be attributed to similar factors, namely, the wake of the viscous drop potentially inducing fewer instances of droplets aligning side-by-side and more instances of vertically stable configurations. DNS at higher $Ga$ and $\lambda$ would be necessary to confirm or not the presence of the wake trapping effect. %To conclusively confirm or refute the presence of this wake-trapping effect, it would be necessary to conduct DNS at higher $Ga$ and $\lambda$ values.
