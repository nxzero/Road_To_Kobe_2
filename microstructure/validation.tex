...
\begin{align*}
    ...
    && N_b =125,
\end{align*}
with mesh resolutions of $d/\Delta = 7,\; 15,\; 25,\; 30 \; 60$. 
...
For high inertia regimes ($Ga = 80$) the boundary layers at the droplet interfaces require the fine grid to be resolved compared to the low inertia cases. 
...
\begin{figure}[h!]
    \centering
    \includegraphics[height = 0.3\textwidth]{image/HOMOGENEOUS_final/CA/tr.pdf}
...
The good agreement between the case  $d/\Delta = 7.37$ and $d/\Delta = 25.21$ is partially fortuitous.
...
% \end{figure}
% In \ref{fig:Re} (right) we display the running-averaged drift velocity as a function of time, for five mesh resolutions. 
% Now let's verify the good convergence of the results for all our cases. 
