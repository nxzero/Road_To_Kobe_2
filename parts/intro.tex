...
\begin{figure}[h!]   
    ...
    \begin{tikzpicture}[font=\footnotesize,very thick, scale = 0.9] 
        ...
        \node (img1) at (-0.35\textwidth,0.25\textwidth) {\includegraphics[height=0.22\textwidth]{image/coalesence2.jpg}};
...
We would now like to provide a general picture of the different methodologies employed to model dispersed flows, whether it is for two-fluid flows, for granular material, or (as it is the case in kinetic theory) molecules. 
...
In opposition, the ``Eulerian-based models'' use an ``Eulerian'' approach to describe either the continuous or dispersed phase, and then we employ an average procedure to derive the macroscopic equations. 
...
Nevertheless, one advantage of this model compared to ``Population-balance-equations'' or ``Kinetic-Theory'', is that the terms of exchanges between phases are consistent. 
...
\citet{ishii1979drag} by compiling various experiments found in the literature proposed values for this coefficient adapted to droplets suspensions, hence providing a drag force coefficient valid for arbitrary $\phi$, $Re$ and $\lambda\approx 1$.
...
Reynolds stress (i.e. the variance of the local continuous phase velocity) models are considerably less frequently proposed in the literature. 
...
% \paragraph*{Part I:} 
The first part focuses on the derivation of the averaged equations as well as the derivation of the closure problem. 
...
% The goal of this chapter is to provide a general approach to derive averaged equations, so that the reader obtains an understanding of the mathematical structure of these equations. 
...
Then, we demonstrate how to derive the corresponding conditionally averaged equations to obtain these terms. 
...
% \paragraph*{Part II:} 
In the second part we propose closure models for the averaged equations. 
...
\ref{chap:mono-disperse} and \ref{chap:pseudoturbulence} address the problem of deriving the interphase drag force and the Reynolds stress models, respectively. 
Based on the theory and simulation results we build models taking into account arbitrary $\lambda$, $Re$ ($< 100$), and $\phi$ ($< 0.2$). 



% \paragraph*{Part III:} 
The last part focuses on the study of the droplet-droplet interactions. 
