...
\chapter{Derivation of the closure problem for spherical particles. }
\label{chap:daniel2}
\chapabstract{
...
Notably, we show that the assumptions made by \citet{batchelor1972sedimentation} to derive its formula are not sufficient to arrive at the actual expression given by its formulation.
Notably, this explains the non-converging issue sometimes encountered using this relation \citep{caflisch1985variance}. 
...
It is shown in \citep{fintzi2024buoyancy} that as predicted by \citet{zhang2023evolution}, the second moment of the nearest particle pair distribution, noted \textbf{R}, can quantify microstructure features such as clusters and layers of particles.
...
We conducted Direct Numerical Simulations (DNS) of statistically steady state mono-disperse buoyant emulsions, over a broad range of \textit{Galileo} numbers ($Ga$), particle volume fractions ($\phi$), and viscosity ratios ($\lambda$). 
...
Overall the mean time of interaction $\tau_p$ seems to be a crucial quantity in the modeling of the relative kinematics   of particles in dispersed two-phase flows.
