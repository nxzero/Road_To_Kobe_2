...
\chapabstract{
    In this chapter we propose to expose and analyze the minimal set of equations to describe emulsions. 
}
\minitoc
...
\label{chap:daniel2}
\chapabstract{In this chapter we revisit the closure problem for spherical droplets in Stokes flows. 
We propose a generic routine to derive \textit{conditional averaged} Navier-Stokes equations and show that it is consistent with the previous studies in the literature. 
Additionally, we demonstrate that our method can be extended to more complicated situation. 
} 
\minitoc
