...
\subsubsection{Relation with the pseudo turbulent particle Reynolds stress and further modeling possibly}

In addition to provides physical explanation the fields $\textbf{u}_p^d = \textbf{u}^r_p - \textbf{u}_p$ is of great importance to study the particle phase fluctuation tensor $\avg{\delta_i \textbf{u}_i' \textbf{u}_i'}$ which is of crucial importance in multiphase flow modeling. 
...
\begin{equation*}
    ...
    d\textbf{r}
\end{equation*}
where, $\textbf{F} = \avg{\delta_j\delta_i h_{ij}(\textbf{u}_i - \textbf{u}_p^{nst})(\textbf{u}_i - \textbf{u}_p^{nst})}$. 
...
At first order in $\phi$ no more than two particle interaction arise, so the former term represents the majority of the particle phase fluctuation. 
This could lead to a model for the particle phase stress. 
...
\tb{
    ...
    However, the entire description of the velocity field is not necessary. 
    Instead, one may use the relation 
    \begin{equation*}
        \textbf{w}_p^r P_r(\textbf{r})
        = \int_{\mathbb{R}^3} 
        \textbf{w}_p^r P_r (\textbf{x})
        \delta( \textbf{r}-  \textbf{x})
        d\textbf{x}
    \end{equation*}
    Which upon using the relation, $\delta(\textbf{r} - \textbf{x}) = \delta(\textbf{r}) - \textbf{x} \cdot \pddx \delta(\textbf{r})\cdot$, gives
    ...
    \begin{equation*}
        ...
        \delta( \textbf{r})
        \int_{\mathbb{R}^3} 
        \textbf{x} \cdot \textbf{w}_p^r P_r (\textbf{x})
        d\textbf{x}
    \end{equation*}
