...
%DIF DEL ../Parper_micro/microstructure_old.tex   Sun Jul 28 08:18:53 2024
%DIF ADD microstructure_new.tex                   Wed Jul 31 08:45:57 2024
...
In \DIFdelbegin \DIFdel{contrast to most previous studies, }\DIFdelend \DIFaddbegin \DIFadd{this study }\DIFaddend we keep the \DIFdelbegin \DIFdel{number of droplets constant while changing the volume fraction $\phi$}\DIFdelend \DIFaddbegin \DIFadd{particle equivalent diameter $d$ constant for all DNS}\DIFaddend . 
We then modify the domain size $\mathcal{L}$ \DIFdelbegin \DIFdel{accordingly}\DIFdelend \DIFaddbegin \DIFadd{or the number of particle $N_b$ accordingly to obtain the desired $\phi$}\DIFaddend . 
...
Therefore, we \DIFdelbegin \DIFdel{use a number of particles per domain of $N_b = 125$, }\DIFdelend \DIFaddbegin \DIFadd{set $\mathcal{L}/d = 10$, which is }\DIFaddend roughly what \citet{hidman2023assessing} used for their DNS of fully-periodic buoyant rising bubbles.
\DIFaddbegin \DIFadd{Likewise, we use a number of particles per domain of at least $N_b = 160$ for all our cases, which introduces the need for a larger domain ($\mathcal{L}/d = 20$) for the dilute cases. 
}\DIFaddend Moreover, each DNS lasts for a time: \DIFdelbegin \DIFdel{$t^*_\text{end} = 1500 \sqrt{d/g}$}\DIFdelend \DIFaddbegin \DIFadd{$t^*_\text{end} = 400 \sqrt{d/g}$ for the larger domains ($\mathcal{L}/d=20$) and $t^*_\text{end} = 1000 \sqrt{d/g}$ for the smaller domain.
It is shown in \ref{ap:validation} that these parameters are sufficient to obtain well converged statistics}\DIFaddend .  
\begin{table}[h!]
    \centering
...
Data for each Lagrangian quantity were gathered every \DIFdelbegin \DIFdel{10 }\DIFdelend \DIFaddbegin \DIFadd{20 }\DIFaddend simulation timesteps. 
...
On average, \DIFdelbegin \DIFdel{$200,000$ }\DIFdelend \DIFaddbegin \DIFadd{$50,000$ }\DIFaddend timesteps are performed during a simulation with \DIFdelbegin \DIFdel{$N_b = 125$ droplets}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{L}/d=20$ and $200,000$ with $\mathcal{L}/d=10$}\DIFaddend . 
This results in a total of \DIFdelbegin \DIFdel{$E = 2,500,000$ events }\DIFdelend \DIFaddbegin \DIFadd{$E = 400,000$ and $2,000,000 $ events for $\phi = 0.01$ to $\phi=0.2$, respectively}\DIFaddend .
...
\begin{figure}[h!]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[height = 0.3\textwidth]{image/HOMOGENEOUS_NEW/PA/Ry.pdf}
%DIFDELCMD <     %%%
...
\begin{figure}[h!]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[height=0.21\textwidth]{image/HOMOGENEOUS_NEW/Dist/Pnst_l_10_Ga_100_PHI_0_05.pdf}
%DIFDELCMD <     \includegraphics[height=0.21\textwidth]{image/HOMOGENEOUS_NEW/Dist/Pnst_l_1_Ga_100_PHI_0_05.pdf}
%DIFDELCMD <     \includegraphics[height=0.21\textwidth]{image/HOMOGENEOUS_NEW/Dist/Pnst_l_10_Ga_100_PHI_0_2.pdf}
%DIFDELCMD <     \includegraphics[height=0.21\textwidth]{image/HOMOGENEOUS_NEW/Dist/Pnst_l_1_Ga_100_PHI_0_2.pdf}
%DIFDELCMD <     %%%
...
\begin{figure}[h!]
   \centering
   \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.4\textwidth]{image/HOMOGENEOUS_NEW/P_PHI_5_l_10_Ga_100.png}
%DIFDELCMD <    \includegraphics[width=0.4\textwidth]{image/HOMOGENEOUS_NEW/P_PHI_5_l_1_Ga_100.png}
%DIFDELCMD <    %%%
...
\begin{figure}[h!]
    \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=0.3\textwidth]{image/HOMOGENEOUS_NEW/Dist/Pr_l_1_Ga_10.pdf}
%DIFDELCMD <     \includegraphics[height=0.3\textwidth]{image/HOMOGENEOUS_NEW/Dist/Pr_l_10_Ga_10.pdf}
%DIFDELCMD <     \includegraphics[height=0.3\textwidth]{image/HOMOGENEOUS_NEW/Dist/Pr_l_1_Ga_100.pdf}
%DIFDELCMD <     \includegraphics[height=0.3\textwidth]{image/HOMOGENEOUS_NEW/Dist/Pr_l_10_Ga_100.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=0.3\textwidth]{image/HOMOGENEOUS_final/Dist/Pr_l_1_Ga_10.pdf}
...
Consequently, for $Ga = 80$ and $\phi \le 0.05$, hydrodynamic interactions seem to be more significant for $\lambda=10$ than $\lambda=1$, as we observe lower values of $P_\text{r-nst}/P_\text{nst}^\text{th}$ }\DIFaddend in the \DIFdelbegin \DIFdel{inertial and low viscous regime, }\DIFdelend \DIFaddbegin \DIFadd{close vicinity of }\DIFaddend the \DIFdelbegin \DIFdel{nearest neighbor is more likely to be located at near contact with the }\textit{\DIFdel{test particle}}%DIFAUXCMD
\DIFdel{.  
...
This phenomenon is particularly pronounced for the case \DIFdelbegin \DIFdel{$Ga = 100$ }\DIFdelend \DIFaddbegin \DIFadd{$Ga = 80$ }\DIFaddend and $\lambda =1$. 
...
\begin{figure}
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[height = 0.3\textwidth]{image/HOMOGENEOUS_NEW/PA/rm.pdf}
%DIFDELCMD <   %%%
...
\begin{figure}[h!]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[height=0.3\textwidth]{image/HOMOGENEOUS_NEW/PA/trR.pdf}
%DIFDELCMD <     \includegraphics[height=0.3\textwidth]{image/HOMOGENEOUS_NEW/PA/Axx.pdf}
%DIFDELCMD <     %%%
...
Indeed $\bm\delta:\textbf{R}/r_m^2$ increases until $Ga = 25$ and then decreases until \DIFdelbegin \DIFdel{$Ga = 100$}\DIFdelend \DIFaddbegin \DIFadd{$Ga = 80$}\DIFaddend . We also note that the distance to the nearest neighbor is not significantly affected by the viscosity ratio, particularly when dealing with high-volume fractions. 
...
%DIFDELCMD <         \draw[dashed] ($(-1,-1.6) + (-10:3 and 2)$(P) arc
%DIFDELCMD <         (-10:155:3 and 2);
...
%DIFDELCMD <         \draw[dashed] ($(10,-2) + (-10:3 and 2)$(P) arc
%DIFDELCMD <         (-10:180:3 and 2);
...
\begin{figure}[h!]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[height = 0.3\textwidth]{image/HOMOGENEOUS_NEW/PA/chi.pdf}
%DIFDELCMD <     %%%
...
\begin{align*}
    ...
    && \zeta = 1.11,
    && Bo = 0.2,
    && Ga = \DIFdelbegin \DIFdel{100}\DIFdelend \DIFaddbegin \DIFadd{80}\DIFaddend ,
    && \phi = 0.1,
    && N_b =125,
\end{align*}
with mesh resolutions of \DIFdelbegin \DIFdel{$d/\Delta = 5,\; 10,\; 18,\; 37$}\DIFdelend \DIFaddbegin \DIFadd{$d/\Delta = 7,\; 15,\; 25,\; 30 \; 60$}\DIFaddend . 
...
For high inertia regimes (\DIFdelbegin \DIFdel{$Ga = 100$}\DIFdelend \DIFaddbegin \DIFadd{$Ga = 80$}\DIFaddend ) the boundary layers at the droplet interfaces require the fine grid to be resolved compared to the low inertia cases. 
...
\DIFaddbegin \DIFadd{Additionally, the lower }\textit{\DIFadd{Bond}} \DIFadd{number employed here ($Bo=0.2$ instead of $Bo =0.5$) requires a higher mesh quality as well. 
}\DIFaddend For these reasons, we suppose that this case might require the finest grid among all other cases presented in this study. 
...
\begin{figure}[h!]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[height = 0.3\textwidth]{image/HOMOGENEOUS_NEW/VAL/Re.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height = 0.3\textwidth]{image/HOMOGENEOUS_final/CA/tr.pdf}
    \includegraphics[height = 0.3\textwidth]{image/HOMOGENEOUS_final/CA/Re.pdf}
    \DIFaddendFL \caption{
...
In the legend we display the value of the mesh resolution. 
    ...
    \label{fig:Re}
\end{figure}
In \ref{fig:Re} \DIFaddbegin \DIFadd{(right) }\DIFaddend we display the running-averaged drift velocity as a function of time, for \DIFdelbegin \DIFdel{four }\DIFdelend \DIFaddbegin \DIFadd{five }\DIFaddend mesh resolutions. 
The results are not as independent of the mesh resolution as the ordered array validation presented above. 
Indeed, we observe a difference of the rising Reynolds number of about \DIFdelbegin \DIFdel{$5\%$ between the $d/\Delta = 18$ and $d/\Delta = 37$ }\DIFdelend \DIFaddbegin \DIFadd{$ 3\%$ between the $d/\Delta = 25$ and $d/\Delta = 60$ }\DIFaddend cases which is notable.
We recall that this \DIFdelbegin \DIFdel{$5\%$ }\DIFdelend \DIFaddbegin \DIFadd{$3\%$ }\DIFaddend error will eventually be lower for all other cases. 
\DIFaddbegin 
...
}\DIFaddend The good agreement between the case  \DIFdelbegin \DIFdel{$d/\Delta = 10$ and $d/\Delta = 18$ }\DIFdelend \DIFaddbegin \DIFadd{$d/\Delta = 7.37$ and $d/\Delta = 25.21$ }\DIFaddend is partially fortuitous.
